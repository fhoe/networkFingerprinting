...
#include "fprinting/scamper_fprinting.h"
...
static char *fp_codename(uint8_t code, uint8_t r) {
	static char names[6][23] = {"echo reply", "time-exp", "timestamp reply", "time fragment exceeded", "port unreacheable", "unknown"};
	if(code == 0)
		return names[0];
	else if(code == 11 && r == 0)
		return names[1];
...
static void dump_fprinting_reply(const scamper_fprinting_t *fprinting,
			    const scamper_fprinting_reply_t *reply)
{
  ...
  if(SCAMPER_FPRINTING_REPLY_IS_ICMP(reply))
    {
      printf("[icmp_type=%d(%s)] | [icmp_code=%d]]\n", reply->icmp_type, 
			fp_codename(reply->icmp_type, reply->icmp_code), reply->icmp_code);
    }
  else if(SCAMPER_FPRINTING_REPLY_IS_TCP(reply))
    {
		if(fprinting->istos)
			printf("[tcp_tos=%08x] | ", reply->reply_tos);
		if(fprinting->isipdf)
			printf("[tcp_df=%01x] | ", reply->reply_df);
      printf("[tcp_flags%02x]]\n", reply->tcp_flags);
      printf("[tcp_mss:%d]", reply->reply_tcp_mss);
      printf("[tcp_win:%d]\n", reply->reply_tcp_win);
...
static char *vreply(scamper_fprinting_reply_t *reply) {
   static char buf[4] = "";
   if(reply == NULL)
      return strdup("-1");
   else if(reply == (scamper_fprinting_reply_t *)-1)
      return strdup("#");
   sprintf(buf, "%d", reply->os_ttl); 
   return strdup(buf);
}

static char *dfreply(scamper_fprinting_reply_t *reply) {
   static char buf[4] = "";
   if(reply != NULL && reply != (scamper_fprinting_reply_t *)-1) {
      sprintf(buf, "%d", reply->reply_df); 
      return strdup(buf);
   }
   return strdup("-1");
}

static char *tosreply(scamper_fprinting_reply_t *reply) {
   static char buf[4] = "";
   if(reply != NULL && reply != (scamper_fprinting_reply_t *)-1) {
      sprintf(buf, "%d", reply->reply_tos); 
      return strdup(buf);
   }
   return strdup("-1");
}

static char *mplsreply(scamper_fprinting_reply_t *reply){
   static char buf[4] = "";
   uint8_t contained_label = 0;
   if(reply != NULL && reply != (scamper_fprinting_reply_t *)-1) {
      
      if(reply->reply_ext != NULL)
         contained_label = 1;
      
      sprintf(buf, "%d", contained_label); 
      return strdup(buf);
   }
   return strdup("-1");
}

static void dump_fprinting(scamper_fprinting_t *fprinting)
{
  ...
  scamper_addr_tostr(fprinting->src, buf, sizeof(buf));
  printf("fprinting from %s", buf);
  if(fprinting->ismulti == 0) {
     scamper_addr_tostr(fprinting->dst, buf, sizeof(buf));
     printf(" to %s\n", buf);
  } else {
     printf(" to severeal ip's\n");
  }

  dump_timeval("start", &fprinting->start);

  printf(" method: %s, protocol: %s, #find: %d, #probe : %d, source port: %d, ittl: %d\n", fprinting->isping?
		"ping" : "traceroute", fprinting->pbmode? "icmp echo" : "tcp syn",
   ...
   if(fprinting->istos)
		printf("get tcp/ip syn tos in reply\n");

  printf("stop-reason: ");
  switch(fprinting->stop_reason)
    {
    case SCAMPER_FPRINTING_STOP_NONE:
      printf("none"); break;

    case SCAMPER_FPRINTING_STOP_COMPLETED:
      printf("done"); break;

    case SCAMPER_FPRINTING_STOP_ERROR:
      printf("sent to errno %d", fprinting->stop_data); break;

    case SCAMPER_FPRINTING_STOP_HALTED:
      printf("halted"); break;

    default:
      printf("reason 0x%02x data 0x%02x",
	      fprinting->stop_reason, fprinting->stop_data);
      break;
    }
  printf("\n\n***dumping replies:\n");
  ...
  reply = fprinting->fprinting_replies;
	...
	while(reply != NULL)
    {
	  dump_fprinting_reply(fprinting, reply);
		reply = reply->next;
    }

  printf("\n\n***computed vectors for ip's:\n");
  printf("? = unknown or not asked, # = irrelevant or inconsistant\n");
  printf("(ip, ttl-exp ittl, tcp ittl, icmp echo ittl, icmp timestamp ittl, icmp port unreach ittl, ttl-exp size, ip df, ip tos, mpls)\n");
  ...
  for(l = 0; l < j; l++) {
     if(els[l].spec != NONE && els[l].spec != DELETED) {
        ...
        if(ip == NULL) {
           printf("%s, -1, -1, -1, -1, -1, -1, -1, -1, -1\n", scamper_addr_tostr((scamper_addr_t *)els[l].key, buf, 255));
        } else {
           if(ip->ttlexp && ip->ttlexp != (scamper_fprinting_reply_t *)-1)
              sprintf(u, "%d", ip->ttlexp->reply_size);
           else
              sprintf(u, "-1");
           printf("(%17s, %3s, %3s, %3s, %3s, %3s, %5s, %1s, %8s, %1s)\n", scamper_addr_tostr((scamper_addr_t *)els[l].key, buf, 255), 
              (a = vreply(ip->ttlexp)), (b = vreply(ip->tcp)), (c = vreply(ip->echo)), (d = vreply(ip->time)),
              (e = vreply(ip->ptunreach)), u, (f = dfreply(ip->tcp)), (g = tosreply(ip->tcp)), (h = mplsreply(ip->ttlexp)));
      ...
      while(scamper_file_read(file, filter, &type, &data) == 0)
	{
	  ...
	  switch(type)
	    ...
	    case SCAMPER_FILE_OBJ_SNIFF:
		 ...
		 case SCAMPER_FILE_OBJ_FPRINTING:
	      dump_fprinting(data);
	      break;
