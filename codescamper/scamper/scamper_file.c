...
#include "fprinting/scamper_fprinting_warts.h"
#include "fprinting/scamper_fprinting_text.h"
...
struct handler
{
  ...
  int (*write_tracebox)(const scamper_file_t *sf,
		    const struct scamper_tracebox *tracebox);

  int (*write_sniff)(const scamper_file_t *sf,
		     const struct scamper_sniff *sniff);
		     
  int (*write_fprinting)(const scamper_file_t *sf,
           const struct scamper_fprinting *fprinting);
...
static struct handler handlers[] = {
  {"text",                                 /* type */
   ...
   scamper_file_text_tracebox_write,       /* write_tracebox */
   NULL,                                   /* write_sniff */
   scamper_file_text_fprinting_write,      /* write_fprinting */
   NULL,                                   /* free_state */
  },
  {"arts",                                 /* type */
  ...
  {"warts",                                /* type */
   ...
   scamper_file_warts_tracebox_write,      /* write_tracebox */
   scamper_file_warts_sniff_write,         /* write_sniff */
   scamper_file_warts_fprinting_write,     /* write_fprinting */
...
int scamper_file_write_tracebox(scamper_file_t *sf,
			    const struct scamper_tracebox *tracebox)
{
  if(sf->type != SCAMPER_FILE_NONE && handlers[sf->type].write_tracebox != NULL)
    {
      return handlers[sf->type].write_tracebox(sf, tracebox);
    }
  return -1;
}
...
int scamper_file_write_fprinting(scamper_file_t *sf,
			     const struct scamper_fprinting *fprinting)
{
  if(sf->type != SCAMPER_FILE_NONE && handlers[sf->type].write_fprinting != NULL)
    {
      return handlers[sf->type].write_fprinting(sf, fprinting);
    }
  return -1;
...
int scamper_file_write_obj(scamper_file_t *sf, uint16_t type, const void *data)
{
  static int (*const func[])(scamper_file_t *sf, const void *) = {
    ...
    (write_obj_func_t)scamper_file_write_tracebox,
    (write_obj_func_t)scamper_file_write_sting,
    (write_obj_func_t)scamper_file_write_sniff,
    (write_obj_func_t)scamper_file_write_fprinting,
  };
