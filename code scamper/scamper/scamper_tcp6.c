...
static size_t tcp_md5(uint8_t *buf, const scamper_probe_t *probe)
{
  buf[0] = 19;
  buf[1] = 18;
  int i;
  for (i=0;i<4;i++)
    bytes_htonl(buf+2+(4*i), probe->pr_tcp_md5digest[i]);
  return 18;
}

static size_t tcp_auth(uint8_t *buf, const scamper_probe_t *probe)
{
  buf[0] = 29;
  buf[1] = 20;
  buf[2] = probe->pr_tcp_authkeyid;
  buf[3] = probe->pr_tcp_authrnextkeyid;  
  int i;
  for (i=0;i<4;i++)
    bytes_htonl(buf+2+(4*i), probe->pr_tcp_authmac[i]);
  return 20;
}

static size_t tcp_mpcapable(uint8_t *buf, const scamper_probe_t *probe)
{
  buf[0] = 30;
  buf[1] = 12;
  buf[2] = 0;
...
static size_t tcp_mpjoin(uint8_t *buf, const scamper_probe_t *probe)
{
  buf[0] = 30;
  buf[1] = 12;
  buf[2] = 16;
  buf[3] = 0;
  bytes_htonl(buf+4, probe->pr_tcp_mpjoin);
  bytes_htonl(buf+8, probe->pr_tcp_mpjoin2);
  return 12;
}
...
size_t scamper_tcp6_hlen(scamper_probe_t *pr)
{
  ...
  if(pr->pr_tcp_flags & TH_SYN)
	...
	tcphlen += 2;
      if(pr->pr_tcp_mpcapable != 0)
	tcphlen += 12;
      if(pr->pr_tcp_mpjoin != 0)
        tcphlen += 12;
      ...
      if(probe->pr_tcp_flags & TH_SYN)
	{
	  ...
	  if((probe->pr_tcp_opts & SCAMPER_PROBE_TCPOPT_SACK) != 0)
	    tcphlen += tcp_sackp(buf+ip6hlen+tcphlen);
         if(probe->pr_tcp_mpcapable != 0)
	    tcphlen += tcp_mpcapable(buf+ip6hlen+tcphlen, probe);
         if(probe->pr_tcp_mpjoin != 0)
            tcphlen += tcp_mpjoin(buf+ip6hlen+tcphlen, probe);
         if(probe->pr_tcp_wscale != 0)
	    tcphlen += tcp_wscale(buf+ip6hlen+tcphlen, probe->pr_tcp_wscale);
         if(probe->pr_tcp_md5 != 0)
	    tcphlen += tcp_md5(buf+ip6hlen+tcphlen, probe);
         if(probe->pr_tcp_auth != 0)
	    tcphlen += tcp_auth(buf+ip6hlen+tcphlen, probe);
	}
